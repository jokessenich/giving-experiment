...
export function JourneyMap({ stops }: { stops: MapStop[] }) {
    ...
    return (
      <div className="journey-map">
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
          <rect width={W} height={H} fill="#efe4c8" />
          <text x={W / 2} y={H / 2}
            fontFamily="Caveat, cursive" fontSize="22"
            fill="#8a7d63" textAnchor="middle">
  ...
  for (let i = 0; i <= H / gridSize; i++) {
    grid.push(<line key={`gh${i}`} x1={0} y1={i * gridSize} x2={W} y2={i * gridSize} stroke="#d8c894" strokeWidth={0.6} />);
  }
  for (let i = 0; i <= W / gridSize; i++) {
    grid.push(<line key={`gv${i}`} x1={i * gridSize} y1={0} x2={i * gridSize} y2={H} stroke="#d8c894" strokeWidth={0.6} />);
  }
    ...
    <div className="journey-map">
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
        ...
        {projected.map((p, i) => {
          ...
          let fill = '#5e7048';
          if (stop.is_starter) fill = '#b88838';
          else if (stop.ended_chain) fill = '#a04829';
          else if (isLast) fill = '#485837';
          const labelAlign: 'start' | 'end' = p.x > W * 0.7 ? 'end' : 'start';
          const dx = p.x > W * 0.7 ? -10 : 10;
          return (
            <g key={i}>
              {isLast && !stop.ended_chain && (
                <circle cx={p.x} cy={p.y} r={13} fill="none" stroke="#485837" strokeWidth={1} opacity={0.4} />
              )}
              <circle cx={p.x} cy={p.y} r={r} fill={fill} />
              <text
                x={p.x + dx} y={p.y - 12}
                fontFamily="Caveat, cursive" fontSize={16}
                fill="#4a3f2c" textAnchor={labelAlign}
                fontWeight={500}
              >
                {stop.city.split(',')[0]}
              </text>
